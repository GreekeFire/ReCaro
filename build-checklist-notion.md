...
- [ ] Add header row with exactly 13 columns in order: `A=Timestamp, B=VA Code, C=Product Title, D=Product Description, E=Shopee Link, F=Source Cost, G=Drive Folder, H=Sell Price, I=Assigned Lister, J=Status, K=Carousell URL, L=Date Posted, M=Notes`
...
- [ ] Add header row with 6 columns: `A=VA Code, B=Role, C=Daily Target, D=Status, E=Rejection Count, F=Contact`
...
- [ ] Add header row: `Trial Code, Timestamp, Title 1, Shopee Link 1, Drive Folder 1, Title 2, Shopee Link 2, Drive Folder 2, Title 3, Shopee Link 3, Drive Folder 3, Title 4, Shopee Link 4, Drive Folder 4, Title 5, Shopee Link 5, Drive Folder 5, Score 1, Score 2, Score 3, Score 4, Score 5, Average Score, Pass/Fail`
- [ ] Freeze row 1 as header.

**ListerTrials Tab**

- [ ] Add header row: `Trial Code, Timestamp, Carousell URL 1, Carousell URL 2, Carousell URL 3, Pass/Fail`
- [ ] Freeze row 1 as header.
...
- [ ] QC check — title length: 190–225 characters. Return error on fail, write nothing to sheet.
- [ ] QC check — description length: 100+ characters.
- [ ] QC check — cost: numeric, not zero, not blank.
- [ ] QC check — Shopee URL starts with `https://shopee.sg/`
- [ ] QC check — Drive folder contains `drive.google.com`
- [ ] QC check — duplicate Shopee URL: call `isDuplicateShopeeURL(shopeeUrl)`
- [ ] On all checks passed: calculate sell price with `getSellPrice(cost)`
- [ ] Call `assignLister()` — if null, send immediate alert email to Quinn and return error to VA
...
- [ ] Write `rebalanceTasks(inactiveListerCode)` — read all Pending rows assigned to that lister, call `assignLister()` for each, update column I with new assignment. Never touch Done rows.

**Time Trigger: Nightly Archive**

- [ ] Write `archiveCompletedRows()` — call `isRestDay()` first, return if true. Move all Done rows older than 24 hours to Archive tab. Delete those rows from Submissions (delete bottom-up to preserve indices).
- [ ] Add trigger: time-driven → day timer → between 11pm–midnight → function: `archiveCompletedRows`

**Time Trigger: Mid-Day Lister Alert**

- [ ] Write `midDayListerAlert()` — call `isRestDay()` first. For each Active lister, if completions today < 25% of their pending count, include in alert email to Quinn.
- [ ] Add trigger: time-driven → hour timer → between 2pm–3pm SGT → function: `midDayListerAlert`

**Time Trigger: Daily Performance Alert**

- [ ] Write `dailyPerformanceAlert()` — call `isRestDay()` first. Build and send email to Quinn containing:
- [ ] RVA section: submissions today vs target per researcher. Zero-submission RVAs flagged at top.
- [ ] LVA section: completions today vs assigned pending queue per lister. Under-25% listers flagged at top.
- [ ] Queue section: total pending count, days of capacity remaining, overflow warning if pending > 2× daily capacity.
- [ ] Format failures: any Carousell URLs that failed format validation today.
- [ ] Flag summary: all rows where column M was written today — researcher VA code, lister who flagged, reason.
- [ ] Spot-check URLs: at least 1 Done URL per Active lister. Extra draws from listers in first 14 days or with recent flags. Cap at 5 URLs total.
...
- [ ] Write `weeklyPerformanceSummary()` — runs Saturdays only. For each Active RVA: count flagged rows in Submissions + Archive for past 7 days (weekly rejection count), count total submissions, calculate rejection rate (show only if 10+ submissions). Flag RVAs with 3+ rejections or >15% rate. Flag RVAs with zero submissions for 3+ consecutive days. For each Active LVA: total completions for week and daily average. Total listings posted across all listers. Send email to Quinn.
...
- [ ] Field 1: Trial Code — on entry, validate expiry and duplicate via Apps Script and display error immediately if failed
...
- [ ] Field 1: Trial Code with same expiry and duplicate validation
...
**COMPONENT 5 — SOPs (Notion Pages)**

- [ ] **[OWNER INPUT]** Create both pages inside your ReCaro Notion workspace.

**Researcher SOP**

- [ ] Create Notion page: "Researcher VA — Standard Operating Procedure"
...
- [ ] Section 5: Image rules — 5–10 images, clean cover with cleanup.pictures if watermark, logo, or promo text present
- [ ] Section 6: Common rejection reasons and how to avoid them
- [ ] Set sharing: "Anyone with the link can view"
- [ ] Copy shareable link — this is the SOP URL used in researcher onboarding messages

**Lister SOP**

- [ ] Create Notion page: "Lister VA — Standard Operating Procedure"
...
- [ ] Set sharing: "Anyone with the link can view"
- [ ] Copy shareable link — this is the SOP URL used in lister onboarding messages
...
- [ ] Confirm Netlify CLI is installed and authenticated
...
- [ ] Print the URL to console and clean up the temp subfolder after deploy
...
- [ ] Test full researcher submission flow: submit via va-form → confirm row appears in Submissions tab with correct sell price and lister assignment
